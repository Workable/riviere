...
module.exports = (incomingMessage, reqId = undefined, opts = {}) => {
  ...
  const { maxQueryChars, maxHrefChars, maxPathChars } = opts;
  ...
  const shouldSliceQuery = !!(maxQueryChars && query && query.length > maxQueryChars);
  const shouldSlicePath = !!(maxPathChars && path && path.length > maxPathChars);
  const shouldSliceHref = !!(maxHrefChars && href && href.length > maxHrefChars);

  const slicedQuery = shouldSliceQuery ? query.slice(0, maxQueryChars) + '...' : query;
  const slicedPath = shouldSlicePath ? path.slice(0, maxPathChars) + '...' : path;
  const slicedHref = shouldSliceHref ? href.slice(0, maxQueryChars) + '...' : href;
