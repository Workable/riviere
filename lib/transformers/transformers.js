...
const mapInReq = ({ ctx, health, bodyKeys, bodyKeysRegex, headersRegex, maxBodyValueChars }) => {
  if (isHealth(ctx, health)) {
    return Object.assign({}, ctx.logCtx, {
      log_tag: logTags.CATEGORY.INBOUND_REQUEST_HEALTH.TAG
    });
  }
  const meta = extractRequestMeta(ctx, bodyKeys, bodyKeysRegex, maxBodyValueChars, headersRegex);
...
const mapOutRes = ({ ctx, health, bodyKeys, bodyKeysRegex, headersRegex, maxBodyValueChars }) => {
  ...
  const meta = extractRequestMeta(ctx, bodyKeys, bodyKeysRegex, maxBodyValueChars, headersRegex, 'request');
...
function extractRequestMeta(ctx, bodyKeys, bodyKeysRegex, maxBodyValueChars, headersRegex, prefix = '') {
