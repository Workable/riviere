...
const mapInRes = (res, req, startedAt, reqId) => {
  ...
  let contentLength = getContectLength(res.headers);
...
const mapOutReq = (incomingMessage, reqId, opts = {}) => {
  ...
  let contentLength = getContectLength(incomingMessage.headers);
...
const mapOutRes = ({ ctx, health, bodyKeys, headersRegex }) => {
  ...
  let contentLength = getContectLength(ctx.response.headers);
  let userAgent = getUserAgent(ctx.headers);
...
const getContectLength = headers => {
  let contentLength = 0;
