...
const mapInReq = ({ ctx, health, bodyKeys, bodyKeysRegex, headersRegex }) => {
  if (isHealth(ctx, health)) {
    return Object.assign({}, ctx.logCtx, {
      log_tag: logTags.CATEGORY.INBOUND_REQUEST_HEALTH.TAG
    });
  }
  const meta = extractRequestMeta(ctx, bodyKeys, bodyKeysRegex, headersRegex);
  ...
  if (isValidToExtractBody) {
    try {
      ...
      if (opts.bodyKeysRegex) {
        ...
        picked = pickBy(jsonObject, (_, key) => REGEX.test(key));
      } else {
...
const mapOutRes = ({ ctx, health, bodyKeys, bodyKeysRegex, headersRegex }) => {
  ...
  const meta = extractRequestMeta(ctx, bodyKeys, bodyKeysRegex, headersRegex, 'request');
...
function extractRequestMeta(ctx, bodyKeys, bodyKeysRegex, headersRegex, prefix = '') {
  ...
  if (isProperMethod && (bodyKeys || bodyKeysRegex) && typeof ctx.request.body === 'object') {
    ...
    if (bodyKeysRegex) {
      ...
      picked = pickBy(ctx.request.body, (_, key) => REGEX.test(key));
    } else {
