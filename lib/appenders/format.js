...
const unexpectedErr = formats.CATEGORY.UNEXPECTED_ERROR;
...
module.exports = (color = false, date = false) => {
  return {
    ...
    [inRes.TAG]: obj => {
      let prefix = `[${obj.requestId}] ${inRes.MSG} ${obj.method} ${obj.protocol}://${obj.host}${obj.path} ${obj.status} ${obj.duration}ms |`;
      if (date) {
        prefix = `[${new Date().toISOString()}] ${prefix}`;
      }
      if (color) {
    ...
    [unexpectedErr.TAG]: obj => {
      let prefix = `[${obj.context.requestId}] ${obj.context.method} ${obj.context.protocol} ${obj.context.path} |`;
      if (date) {
        prefix = `[${new Date().toISOString()}] ${prefix}`;
      }
      if (color) {
