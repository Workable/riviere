            ...
            if (health) {
                for(let i = 0; i < health.length; i++) {
                    ...
                    if (ctx.request.path === h.path && method === h.method.toUpperCase()) {
                        this.logger.info(`<-- ${ctx.logCtx.method} ${ctx.logCtx.url}`);
                        return;
                    }
                }
            }
        ...
        onOutboundResponse: function({ ctx }) {
            ...
            if (health) { // todo refactor: this is also used in the onInboundRequest function
                for(let i = 0; i < health.length; i++) {
                    const h = health[i];
                    if (ctx.request.path === h.path && method === h.method.toUpperCase()) {
                        this.logger.info(`--> ${ctx.logCtx.method} ${ctx.logCtx.url} ${status} ${duration}ms`);
                        return;
                    }
                }
            }
            ...
            const msg = `[${data.requestId}] --> ${ctx.logCtx.method} ${ctx.logCtx.url} ${data.status} ${data.duration}ms ${this.serialize(data)}`;
            this.logger.info(msg);
        },
        onError({ ctx, err }) {
