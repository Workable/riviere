...
const TAG =  {
    ...
    OUTBOUND_RESPONSE_HEALTH: 'outbound_response_health',
    INBOUND_REQUEST_HEALTH: 'inbound_request_health'
};
...
function onInboundRequest({ ctx }) {
    ...
    // special handle for health endpoints
    const health = this.health;
    if (health) {
        for (let i = 0; i < health.length; i++) {
            const h = health[i];
            if (ctx.request.path === h.path && method === h.method.toUpperCase()) {
                setImmediate(this.logger.info.bind(this.logger), Object.assign({},
                    ctx.logCtx, { log_tag: TAG.INBOUND_REQUEST_HEALTH }));
                return;
            }
        }
    }
...
function onOutboundResponse({ ctx }) {
    ...
    if (health) { // todo refactor: this is also used in the onInboundRequest function
        for (let i = 0; i < health.length; i++) {
            const h = health[i];
            if (ctx.request.path === h.path && method === h.method.toUpperCase()) {
                setImmediate(this.logger.info.bind(this.logger), Object.assign({}, ctx.logCtx,
                    { log_tag: TAG.OUTBOUND_RESPONSE_HEALTH, status, duration}));
                return;
            }
        }
    }
    ...
    const msgObj = Object.assign(data, {
        method: ctx.logCtx.method,
        path: ctx.logCtx.path,
    });
