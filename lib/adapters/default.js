    ...
    apply: (target, thisArg, argumentsList) => {
      ...
      try {
        ...
        if (opts.request && opts.request.enabled) {
          const obfuscateHeadersFound = intersection(
            opts.request.obfuscateHeaders,
            Object.keys(requestOptions.headers)
          );
          const shouldObfuscatePath = obfuscateHeadersFound.some(header => requestOptions.headers[header]);
          if (shouldObfuscatePath) {
            reqObj.href = requestOptions.href.replace(requestOptions.path, '/***');
          }
