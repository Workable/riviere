...
function onInboundRequest({ ctx }) {
  ...
  const { health, bodyKeys, bodyKeysRegex, headersRegex, bodyValuesMaxLength } = this;
  const transformedReq = mapInReq({ ctx, health, bodyKeys, bodyKeysRegex, bodyValuesMaxLength, headersRegex });
...
function onOutboundResponse({ ctx }) {
  ...
  const { health, bodyKeys, bodyKeysRegex, headersRegex, bodyValuesMaxLength } = this;
  const transformedRes = mapOutRes({ ctx, health, bodyKeys, bodyKeysRegex, bodyValuesMaxLength, headersRegex });
