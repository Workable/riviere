...
function onInboundRequest({ ctx }) {
  ...
  const { health, bodyKeys, bodyKeysRegex, headersRegex } = this;
  const transformedReq = mapInReq({ ctx, health, bodyKeys, bodyKeysRegex, headersRegex });
...
function onOutboundResponse({ ctx }) {
  ...
  const { health, bodyKeys, bodyKeysRegex, headersRegex } = this;
  const transformedRes = mapOutRes({ ctx, health, bodyKeys, bodyKeysRegex, headersRegex });
