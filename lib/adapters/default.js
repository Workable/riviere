...
function requestProxy({ logger, level, traceHeaderName, sync = true } = {}) {
  const requestHandler = {
    apply: (target, thisArg, argumentsList) => {
      ...
      const traceHeaderNameLowercase = traceHeaderName.toLowerCase();
      ...
      try {
        ...
        reqId = incomingMessage.headers[traceHeaderNameLowercase]
          ? incomingMessage.headers[traceHeaderNameLowercase]
          : uuidv4();
