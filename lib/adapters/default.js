  ...
  const requestHandler = {
    apply: (target, thisArg, argumentsList) => {
      ...
      try {
        ...
        const blacklistedPaths = opts.blacklistedPaths || [];
        if (!reqObj.protocol || blacklistedPaths.some(p => p === reqObj.path)) {
...
function onInboundRequest({ ctx }) {
  ...
  const transformedReq = mapInReq({ ctx, health, bodyKeys, bodyKeysRegex, headersRegex, maxBodyValueChars });
...
function onOutboundResponse({ ctx }) {
  ...
  const transformedRes = mapOutRes({ ctx, health, bodyKeys, bodyKeysRegex, headersRegex, maxBodyValueChars });
